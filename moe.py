    ...
    def _performBatchStep(self):
        ...
        for i in range(len(self.discountRewards)):
            ...
            if self.nn.linear1.weight.grad is None:
                self.nn.linear1.weight.grad = torch.zeros_like(self.nn.linear1.weight)
            if self.nn.linear2.weight.grad is None:
                self.nn.linear2.weight.grad = torch.zeros_like(self.nn.linear2.weight)
            if self.nn.linear3.weight.grad is None:
                self.nn.linear3.weight.grad = torch.zeros_like(self.nn.linear3.weight)

            if self.nn.linear1.bias.grad is None:
                self.nn.linear1.bias.grad = torch.zeros_like(self.nn.linear1.bias)
            if self.nn.linear2.bias.grad is None:
                self.nn.linear2.bias.grad = torch.zeros_like(self.nn.linear2.bias)
            if self.nn.linear3.bias.grad is None:
                self.nn.linear3.bias.grad = torch.zeros_like(self.nn.linear3.bias)
...
class GatingNetwork(nn.Module):
    '''Gating network to learn weights for combining multiple expert models.'''
...
class MixtureOfExpertsNEURWIN(NEURWIN):
    def __init__(self, stateSize, lr, env, seed, sigmoidParam, numEpisodes, noiseVar,
                 batchSize, discountFactor, saveDir, episodeSaveInterval, num_experts):
        super(MixtureOfExpertsNEURWIN, self).__init__(stateSize, lr, env, seed, sigmoidParam, numEpisodes, noiseVar,
                 batchSize, discountFactor, saveDir, episodeSaveInterval)
        self.experts = [NEURWIN(stateSize, lr, env, seed, sigmoidParam, numEpisodes, noiseVar,
                 batchSize, discountFactor, saveDir, episodeSaveInterval) for _ in range(num_experts)]
        self.gating_network = GatingNetwork(num_experts, stateSize)
        self.gating_optimizer = optim.Adam(self.gating_network.parameters(), lr=lr)

    def takeAction(self, state):
        index = self.nn.forward(state)
        gating_weights = self.gating_network(state)

        # Use gating weights to combine expert decisions
        combined_index = sum(gating_weights[i] * expert.nn.forward(state) for i, expert in enumerate(self.experts))

        if (self.env.episodeTime == 0) and (self.currentEpisode % self.batchSize == 0):
            print(f'new state: {state}')
            self.newMiniBatchReset(index, state)

        sigmoidProb = torch.sigmoid(self.sigmoidParam * (combined_index - self.cost))
        # action = torch.bernoulli(sigmoidProb).item()
        ...
        # Backpropagate through both the selected expert and the gating network
        
        self.optimizer.zero_grad()
        logProb.backward()
        self.gating_optimizer.step()
        for i, expert in enumerate(self.experts):
            expert_grad = [p.grad for p in expert.nn.parameters()]
            # Update expert if it was selected
            if gating_weights[i].item() > 0:
                for p, g in zip(expert.nn.parameters(), expert_grad):
                    if p.grad is not None:
                        p.grad.data = g.data.clone()
                self.optimizer.step()
    ...
    def _performBatchStep(self):
        '''Function for performing the gradient ascent step on accumulated mini-batch gradients for all experts and the gating network.'''

        meanBatchReward = sum(self.discountRewards) / len(self.discountRewards)

        # Update the experts based on the weighted gradients
        for i, reward in enumerate(self.discountRewards):
            adjusted_reward = reward - meanBatchReward
            for expert in self.experts:
                # Make sure gradients are zero before accumulation
                self.optimizer.zero_grad()
                for p in expert.nn.parameters():
                    if p.grad is not None:
                        p.grad.zero_()

                # Accumulate weighted gradients for each expert
                for p in expert.nn.parameters():
                    if p._grad is not None:
                        p._grad += adjusted_reward * p._grad

                # Take a gradient step for each expert
                self.optimizer.step()

        # Clear the gradients for the gating network
        self.gating_optimizer.zero_grad()

        # Update the gating network parameters through gradient ascent
        for p in self.gating_network.parameters():
            if p.grad is not None:
                p.grad.zero_()

        # Accumulate gradients for the gating network
        for i, reward in enumerate(self.discountRewards):
            adjusted_reward = reward - meanBatchReward
            for p in self.gating_network.parameters():
                if p._grad is not None:
        ...
        # Take a gradient step for the gating network
        self.gating_optimizer.step()

        # Reset gradients and rewards for the next batch
        self.discountRewards = []
        self.optimizer.zero_grad()
        self.gating_optimizer.zero_grad()
